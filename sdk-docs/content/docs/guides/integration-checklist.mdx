...
- [ ] Pass the returned `nativeFee` plus the escrow from the relayer quote (`POST /quote`) as `msg.value` to `submitIntent` (see [Pay for storage](/docs/guides/pay-for-storage))
...
- [EVM adapter](/docs/contracts/evm-adapter) for function signatures and wire formats
