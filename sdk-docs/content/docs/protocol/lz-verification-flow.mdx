---
title: "LayerZero verification flow"
description: "Both legs of the round-trip are verified by LayerZero DVNs. The message flow, encodings, and trust boundaries."
---
...
Total: 81 bytes. The commitment is `blobId(32) ++ size(u32) ++ encodingType(u8) ++ storageEpochs(u32) ++ deadline(u64)`, big-endian. No raw blob bytes are on the wire. See [Commitment Format](/docs/contracts/commitment-format).
...
- [Architecture](/docs/protocol/architecture) for the system overview
- [Commitment Format](/docs/contracts/commitment-format) for the forward-message wire layout
- [Sui Executor](/docs/contracts/sui-executor) for the Walrus execution step
- [Contract Interface](/docs/contracts/evm-adapter) for EVM function signatures
- [Blob ingest](/docs/api#blob-ingest-out-of-band) for the out-of-band upload contract
- [Relayer](/docs/protocol/relayer) for the operator that drives both paths
