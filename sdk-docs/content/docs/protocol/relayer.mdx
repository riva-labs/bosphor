---
title: "Relayer"
description: "Run and configure the Bosphor relayer: what it watches, what it signs, its environment, metrics, and durable store queue."
---
...
To measure compute latency and throughput under concurrent load, use the load generator (`npm --prefix relayer run loadgen`); methodology and results are on [Relayer benchmarks](/docs/protocol/benchmarks).
...
- [Architecture](/docs/protocol/architecture) for the full message flow
- [Contract Interface](/docs/contracts/evm-adapter) for Sui function signatures
- [Deployment](/docs/protocol/deployment) for initial setup
- [Testing](/docs/protocol/testing) for running relayer unit tests
