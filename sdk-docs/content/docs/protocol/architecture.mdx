---
title: "Architecture"
description: "How an intent travels from an EVM or Solana origin over LayerZero to Sui and Walrus, and how the proof comes back."
---
...
1. **EVM -> Sui** (LayerZero): `submitIntent` sends `intentId(32) ++ commitment(49)` = 81 bytes (`abi.encodePacked`) through LayerZero v2 OApp messaging. The commitment holds the Walrus blob id, size, encoding, storage duration, and deadline; no raw blob bytes are on the wire. See [Commitment Format](/docs/contracts/commitment-format).
