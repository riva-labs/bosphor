...
description: "Fixes for the setup, deployment, relayer, and cross-chain issues you can hit when running your own Bosphor deployment."
---
...
### The docs portal build fails with broken links

Run `cd sdk-docs && npm run build`. After the static export, the build checks every internal link and every redirect target, and prints each broken one with the page it appears on. Link to pages by their portal path, for example `/docs/protocol/architecture`.
...
- [Quickstart](/docs/quickstart) for your first store on the hosted testnet
- [Self-hosting](/docs/protocol/self-hosting) for running your own deployment
- [Deployment](/docs/protocol/deployment) for deployment steps
- [Relayer](/docs/protocol/relayer) for relayer configuration
- [Known Limitations](/docs/protocol/known-limitations) for protocol constraints
