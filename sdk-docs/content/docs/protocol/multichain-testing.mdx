...
description: "How the EVM and Solana origins are tested end to end through the same Sui and Walrus pipeline."
---
...
`relayer/scripts/cost-curve.ts` prints the all-in escrow for a range of sizes using the real quote engine and WAL cost calculator. It runs offline with **simulated inputs**, not live prices: WAL $0.03, SUI $0.80, ETH $2,500, SOL $100 (override with `WAL_USD`, `SUI_USD`, `ETH_USD`, `SOL_USD`), a return-leg fee of 1.76 SUI, Sui gas of 0.01 SUI, 5 epochs, and a representative Walrus system state (1,000 shards). The live relayer quotes from live prices instead (see [Pay for storage](/docs/guides/pay-for-storage)).
...
The Solana script's phases, requirements, and rate-limit handling are described in the [Testing Guide](/docs/protocol/testing#solana-priced-end-to-end-test).
...
- [Relayer benchmarks](/docs/protocol/benchmarks) for relayer latency, throughput, and fault tolerance
- [Testing Guide](/docs/protocol/testing) for how to run each suite
- [Pay for storage](/docs/guides/pay-for-storage) for the escrow design both chains share
