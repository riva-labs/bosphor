---
title: "Benchmarks"
description: "How fast the relayer processes storage intents and how it behaves under concurrent load and faults, with the command behind every number."
---
...
The relayer exposes two latency histograms (see [Relayer: latency metrics](/docs/protocol/relayer#latency-metrics-and-the-sub-3s-kpi)):
...
Throughput numbers only matter if the relayer never loses or double-pays for an intent under faults. These guarantees come from the durable store queue (see [Relayer: durable store queue](/docs/protocol/relayer#durable-store-queue)):
