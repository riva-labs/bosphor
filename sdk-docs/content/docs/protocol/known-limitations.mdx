...
description: "Current constraints of the Bosphor protocol and what is planned to lift them."
---
...
- The relayer's testnet preset targets Sepolia (EID 40161). With `NETWORK=mainnet` there is no default EVM chain: `EVM_DST_EID` and `EVM_CHAIN_ID` must be set for the chosen chain, or the relayer does not start. See [Relayer](/docs/protocol/relayer#network-presets).

## Related

- [Architecture](/docs/protocol/architecture) for the system design
- [Relayer](/docs/protocol/relayer) for operational details and configuration
- [Deployment](/docs/protocol/deployment) for setup instructions
