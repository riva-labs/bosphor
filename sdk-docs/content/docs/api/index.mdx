...
description: "The public relayer endpoints: the intent feed, quotes, and out-of-band blob upload."
---

The relayer is the only off-chain service an integrator talks to. The SDK calls
it for you, so you only need this page if you build your own client, show a
live status view, or debug a store.

Every endpoint has its own reference page, generated from the relayer's
[OpenAPI 3.1 spec](https://github.com/riva-labs/bosphor/blob/main/relayer/openapi/openapi.yaml),
with request and response schemas, code samples (curl, `fetch` and the
`@bosphor/sdk` call where one exists) and a **try it** console that calls the
hosted testnet relayer straight from your browser. The relayer also serves the
spec itself at [`/openapi.json`](https://api.bosphor.xyz/testnet/openapi.json),
so you can load it into Postman, Insomnia or a client generator.

| Group | Method | Path | What it does |
|-------|--------|------|--------------|
| Health | `GET` | [`/health`](/docs/api/health/get-health) | Liveness of the relayer and its chain connections |
| Pricing | `POST` | [`/quote`](/docs/api/pricing/create-quote) | An all-in price for a store, in the origin chain's native token |
| Ingest | `POST` | [`/blob/encode`](/docs/api/ingest/encode-blob) | Compute a blob id without storing anything |
| Ingest | `POST` | [`/blob/{intentId}`](/docs/api/ingest/ingest-blob) | Upload the file bytes for an on-chain intent, out-of-band |
| Status | `GET` | [`/public/intents`](/docs/api/status/list-intents) | The live lifecycle of recent intents, newest first |
| Meta | `GET` | [`/openapi.json`](/docs/api/meta/get-open-api-json), [`/openapi.yaml`](/docs/api/meta/get-open-api-yaml) | This API description |

Base URLs:

| Network | Base URL |
|---------|----------|
| Testnet (Sepolia, Solana devnet, Sui testnet) | `https://api.bosphor.xyz/testnet` |
| Mainnet | `https://api.bosphor.xyz` |

<Callout type="info" title="Trying it out">
`GET /health`, `POST /quote` and `GET /public/intents` work straight from the
try it console. `POST /blob/{intentId}` needs the id of an intent you have
already submitted on chain (any other id gets a `404`), so upload through the
SDK `store()` call instead of the console. `POST /blob/encode` is rate limited
tightly because it is CPU heavy.
</Callout>

The sections below explain the concepts behind the endpoints: the intent
lifecycle, browser use and rate limits, pricing and the out-of-band upload.
...
[Reference and try it](/docs/api/status/list-intents).
...
[payment flow](/docs/guides/pay-for-storage) for the full model.

### `POST {relayerBaseUrl}/quote`

[Reference and try it](/docs/api/pricing/create-quote).
...
[Reference](/docs/api/ingest/ingest-blob).
...
[Reference and try it](/docs/api/ingest/encode-blob).
...
The `@bosphor/sdk` `store()` flow performs this upload for you (`client.upload(intentId, data)` is also exposed as an escape hatch). See the [EVM guide](/docs/guides/evm) and the [Solana guide](/docs/guides/solana).
