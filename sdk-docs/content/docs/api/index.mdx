...
|--------|------|--------------|
| `GET` | [`/public/intents`](#get-publicintents) | The live lifecycle of recent intents, newest first |
| `POST` | [`/quote`](#post-relayerbaseurlquote) | An all-in price for a store, in the origin chain's native token |
| `POST` | [`/blob/{intentId}`](#post-relayerbaseurlblobintentid) | Upload the file bytes for an on-chain intent, out-of-band |
| `POST` | [`/blob/encode`](#post-relayerbaseurlblobencode) | Compute a blob id without storing anything |
| `GET` | `/health` | Liveness of the relayer and its chain connections |
...
<Callout type="info" title="Interactive reference coming">
An OpenAPI-rendered reference with a "try it" console will replace this
hand-written page. The endpoints and their behaviour below are current.
</Callout>
