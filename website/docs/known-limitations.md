...
## Return proof delivery is operator-driven on testnet

The Sui→origin proof leg has two paths: the pure LayerZero return
(`lz_send_proof`) and an owner-gated fallback (`confirmExecution` on EVM,
`confirm_execution` on Solana) that delivers the identical proof bytes.

The `EUnauthorized` abort that previously blocked the pure-LZ path
([#272](https://github.com/riva-labs/bosphor/issues/272)) was root-caused and
fixed in [#337](https://github.com/riva-labs/bosphor/issues/337): the OApp
executor config on the Sui ULN pointed at the executor package id instead of
the executor worker's CallCap identity, so the ULN created an executor child
call no worker could claim. The on-chain config now inherits the ULN default
executor and `lz_send_proof` completes end to end; the relayer also validates
the effective send config against its worker objects before every first send
and fails with an actionable error instead of an opaque abort.

Delivery of the return packet on the origin chain is performed by the
self-operated Bosphor DVN (verify, commit, `lzReceive`). Until the continuous
return worker is deployed alongside the relayer, delivery is a one-shot
operator action, and the relayer keeps the trusted `confirmExecution` fallback
as disaster recovery so round-trips always complete with the same
`IntentExecuted` proof. The fallback is confined to the relayer and requires no
contract change to remove.
