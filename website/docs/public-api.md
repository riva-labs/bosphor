...
Every relayer serves a machine-readable OpenAPI 3.1 description of this API, with request and response schemas, error codes and examples. Load it into Postman, Insomnia or a client generator:

- `GET /openapi.json` (for example `https://api.bosphor.xyz/testnet/openapi.json`)
- `GET /openapi.yaml`

The source lives in the Bosphor repository at `relayer/openapi/openapi.yaml`, and a relayer test checks it against the running code on every change.
